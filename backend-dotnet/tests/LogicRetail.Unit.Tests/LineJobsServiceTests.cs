...
public sealed class LineJobsServiceTests : IDisposable
{
    ...
    [Fact]
    public async Task SubmitFull_duplicate_ITEM100_returns_business_failure()
    {
        var result = await _sut.SubmitFullAsync(SalesId, Company, WorkerRecId, "ITEM-100", 1, CancellationToken.None);
        var json = JsonSerializer.Serialize(result);
        using var doc = JsonDocument.Parse(json);
    ...
    {
        await _sut.SubmitFullAsync(SalesId, Company, WorkerRecId, "ITEM-100", 1, CancellationToken.None);
